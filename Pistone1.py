...
class PistoneRPi():
    def __init__(self,Debug=0,S=[1,2],P=[18,17]):
        ...
        '''
        if(1):#self.getPin(self.S[1])==0):
            print("UP INIT PHASE ==============>>>>S[1] is 0")
            while(self.getPin(self.S[0])==1):
    ...
    def Down(self):
        i=0
        if(self.getPin(self.S[0])==0):
            print("Down s[0]=0")
            while(self.getPin(self.S[1])==1):
...
def main():
    ...
    print("Start")
    global ts
    Py=PistoneRPi(S=[6,16],P=[18,17])
    k=0
    print("S[0] is",Py.getPin(Py.S[0]))
    print("S[1] is",Py.getPin(Py.S[1]))
    print(Py.Up())
    #print(Py.Down())
    ts=100000
    while(k<1000000):
