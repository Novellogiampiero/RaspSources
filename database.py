...
class db():
    ...
    def MakeJsonTable(self,tablename):
        self.sql =("CREATE TABLE %s(id integer NOT NULL,data jsonb);" %tablename)
    ...
    def GetAllFromtable(self,TableName):
        ...
        #print(self.sql)
        Temp=self.cursor.execute(self.sql)
        print("select return is",Temp)
        #self.conn.commit()
    ...
    '''
    def InsertTestParameterInRefTable(self,TableName,NomeTest,datastarttest,Articolo,Nodo,Numerodibit,Banda,Fc,TestID):
        self.sql = ("INSERT INTO %s (NomeTest,Data, NomeArticolo,NomeRaspberry,NumeroBit,Banda,FrequenzaCamp, TestNr) VALUES (%s, %s, %s, %s, %d,%d, %d, %d);"%(TableName,NomeTest,datastarttest,Articolo,Nodo,Numerodibit,Banda,Fc,TestID))
        #self.sql = ("INSERT INTO %s VALUES (%s, %s, %s, %s, %s,%s, %s, %s);"%(TableName,NomeTest,datastarttest,Articolo,Nodo,Numerodibit,Banda,Fc,TestID))
        print(self.sql)
        self.cursor.execute(self.sql)
        #self.conn.commit()
        
    def InsertParameterInRefTable(self,TableName,NomeTest,datastarttest, Articolo,Nodo,Numerodibit,Banda,Fc):
        TestID=self.GetAllFromtable(TableName)
        self.InsertTestParameterInRefTable(TableName,NomeTest,datastarttest, Articolo,Nodo,Numerodibit,Banda,Fc,TestID)
        self.conn.commit()
        return TestID
